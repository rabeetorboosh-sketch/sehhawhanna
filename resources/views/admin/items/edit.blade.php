<x-app-layout>
        ...
        <form class="smart-form" action="{{route('items.update', $item->id)}}" method="post">
            ...
            <div class="row-2">
                <div class="form-group">
                    ...
                    <input name="name" type="text" value="{{ $item->name }}" autocomplete="off">
                </div>
                <div class="form-group">
                    <label>الوصف</label>
                ...
                <div class="form-group">
                    ...
                    <input name="code" type="text" value="{{ $item->code }}" autocomplete="off">
                </div>
                <div class="form-group">
                    <label>المجموعة الرئيسية</label>
                    <select name="main_group_id">
                        @foreach($mainGroups as $mainGroup)
                            <option value="{{$mainGroup->id}}" {{ $item->main_group_id == $mainGroup->id ? 'selected' : '' }}>{{$mainGroup->name}}</option>
                        @endforeach
                    </select>
                </div>
                <div class="form-group">
                    ...
                    <select name="sub_group_id">
                        @foreach($subGroups as $subGroup)
                            <option value="{{$subGroup->id}}" {{ $item->sub_group_id == $subGroup->id ? 'selected' : '' }}>{{$subGroup->name}}</option>
                        @endforeach
                    </select>
                </div>
    ...
    <script>
        let unitIndex = {{ $item->units->count() }};
