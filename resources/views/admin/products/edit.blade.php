<x-app-layout>
    ...
    <div class="py-12">
        ...
        <form class="smart-form" action="{{route('items.update', $product->id)}}" method="post">
            @csrf
            @method('PUT')
            ...
            <div class="row-2">
                <div class="form-group">
                    ...
                    <input name="name" type="text" value="{{ $product->item->name }}" autocomplete="off">
                </div>
                <div class="form-group">
                    <label>الــــــــوصــــــــف</label>
                ...
                <div class="form-group">
                    ...
                    <select name="main_group_id">
                        @foreach($mainGroups as $mainGroup)
                            <option value="{{$mainGroup->id}}" {{ $product->item->main_group_id == $mainGroup->id ? 'selected' : '' }}>{{$mainGroup->name}}</option>
                        @endforeach
                    </select>
                </div>
                <div class="form-group">
                    ...
                    <select name="sub_group_id">
                        @foreach($subGroups as $subGroup)
                            <option value="{{$subGroup->id}}" {{ $product->item->sub_group_id == $subGroup->id ? 'selected' : '' }}>{{$subGroup->name}}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="form-group" style="border:1px solid gray;border-radius:10px;padding:5px;">
                    ...
                    <div id="units-wrapper">
                        @foreach($product->item->units as $index => $unit)
                            <div class="unit-row">
                                <div class="row-3">
                                    <div class="form-group">
                                        <label>الوحدة</label>
                                    ...
                                    <div class="form-group">
                                        <label>الكمية / العبوة</label>
                                        <input type="number" name="units[{{ $index }}][package]" data-name="package" min="1" value="{{ $unit->package }}">
                                    </div>
                                    <div class="form-group">
    ...
    <script>
        let unitIndex = {{ $product->item->units->count() }};
