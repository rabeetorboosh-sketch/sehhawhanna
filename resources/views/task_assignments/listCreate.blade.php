...
@if(!$tasks->isEmpty())
                <!-- المهام -->
                <div class="form-group assign-section" id="tasks-section">
                    <label>المهمة</label>
                    <div class="search-filter">
                         @foreach($tasks as $task)
                            <div class="item-card">
                                <label>
                                    <input type="checkbox" class="itemsCheck checkbox"  name="tasks[]" value="{{$task->id}}" checked><span>{{$task->item->name }}</span>
                                </label>
                            </div>
                        @endforeach
                    </div>
                        ...
                        @foreach($reports as $report)
                            @foreach($report->items as $item)
                                @if($item->control_unit_id!=null)
                                    <div class="item-card">
                                        <label>
                                            <input type="checkbox" data-date="{{$item->created_at}}" class="itemsCheck checkbox"  name="reports[]" value="{{$item->id}}" checked><span>  {{$item->controlUnit->name .'->'. $item->item?->name }}</span>
                                        </label>
                                    </div>
                                @elseif($item->user_control_unit!=null)
                                    ...
                                    <div class="item-card">
                                        <label>
                                            <input type="checkbox" data-date="{{$item->created_at}} "   class="itemsCheck checkbox"  name="reports[]" value="{{$item->id}}" checked><span> {{$item->user_control_unit .'->'. $item->item?->name}}</span>
                                        </label>
                                    </div>
    ...
    @if(!$monitorings->isEmpty())
                ...
                <!-- الرقابة اليومية -->
                <div class="form-group assign-section" id="monitorings-section"  >
